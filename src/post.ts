...
function determineDeploymentState(jobStatus: string): { state: 'success' | 'failure'; description: string } {
...
async function getJobStatus(octokit: ReturnType<typeof github.getOctokit>, owner: string, repo: string): Promise<JobStatus> {
  try {
    ...
    const mainJob = jobs.jobs.find(job =>
      job.name === github.context.job &&
      job.conclusion !== null
    );

    if (!mainJob) {
      core.warning('Could not find completed main job, checking workflow run status');
    ...
    switch (mainJob.conclusion) {
      ...
      case 'cancelled':
        return 'cancelled';
      default:
        core.warning(`Unknown job conclusion: ${mainJob.conclusion}, assuming failure`);
        return 'failure';
    }
  } catch (error) {
    core.warning(`Failed to get job status: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return 'failure';
  }
}
...
async function run(): Promise<void> {
  try {
    ...
    core.info(`Updating deployment ${savedState.deploymentId} status to ${state}`);
    ...
    const statusParams = savedState.environmentUrl && state === 'success'
      ? { ...baseParams, environment_url: savedState.environmentUrl }
      : baseParams;

    await octokit.rest.repos.createDeploymentStatus(statusParams);

    core.info(`Deployment ${savedState.deploymentId} status updated to ${state}`);
